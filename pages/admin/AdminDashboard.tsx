...
import { 
  ...
  Clock, Wallet, ShieldAlert, Cpu, CheckCircle, Terminal, Zap, Code, Search, Filter, History, Ban, UserCheck, Server, ArrowUpRight, Play, Square, AlertCircle, HardDrive
} from 'lucide-react';
      ...
      <aside className={`fixed inset-y-0 left-0 z-[70] w-72 bg-[#0f172a] border-r border-slate-800/50 transition-transform lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col p-6">
          ...
          <nav className="flex-1 space-y-2">
            ...
            <NavItem to="/a/dashboard/logs" icon={History} label="Canlı Loglar" />
            <NavItem to="/a/dashboard/runtimes" icon={Server} label="Aktif Süreçler" />
            <NavItem to="/a/dashboard/bots" icon={Bot} label="Bot Yönetimi" />
        ...
        <header className="h-20 border-b border-slate-800/50 flex items-center justify-between px-8 bg-[#020617]/50 backdrop-blur-xl shrink-0">
           ...
           <div className="hidden sm:flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master Node: Connected</span>
              </div>
           </div>
           <div className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-xl text-[10px] font-black text-blue-500 uppercase tracking-widest flex items-center gap-2">
             <ShieldAlert size={14} /> Superuser Access
           </div>
        </header>
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard label="Aktif PID" value={stats.activeRuntimes} icon={Server} color="emerald" />
                <StatCard label="Sistem Kaydı" value={stats.logCount} icon={History} color="orange" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-[#0f172a] border border-slate-800 p-8 rounded-[40px] shadow-2xl">
                    <h3 className="text-sm font-black text-white uppercase tracking-widest mb-8 italic flex items-center gap-3">
                        <Activity size={18} className="text-blue-500"/> Küresel Altyapı Trafiği
                    </h3>
                    <div className="h-64 bg-slate-950/50 rounded-3xl border border-slate-900 flex items-end justify-between p-8 gap-2">
                ...
                <div className="bg-[#0f172a] border border-slate-800 p-8 rounded-[40px] shadow-2xl space-y-4">
                    <h3 className="text-sm font-black text-white uppercase tracking-widest mb-8 italic">Server Sağlığı</h3>
                    <HealthBar label="CPU Load" value={14} color="blue" />
                    <HealthBar label="Memory" value={42} color="purple" />
                    <HealthBar label="Disk Space" value={22} color="emerald" />
                    <div className="mt-10 p-6 bg-slate-950/50 rounded-3xl border border-slate-900">
                        ...
                        <p className="text-xs font-black text-emerald-500 mt-2">Active & Responding</p>
                    </div>
                </div>
            </div>
        </div>
...
const StatCard = ({ label, value, icon: Icon }: any) => (
    <div className="bg-[#0f172a] border border-slate-800 p-8 rounded-[40px] group hover:border-blue-500 transition-all">
...
const HealthBar = ({ label, value, color }: any) => (
    <div className="space-y-2">
        ...
        <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden">
            <div className={`h-full bg-${color}-600`} style={{ width: `${value}%` }}></div>
        </div>
    </div>
);

// --- Runtime Monitor (Aktif Botlar) ---
const RuntimeMonitor = () => {
    const [bots, setBots] = useState<BotType[]>([]);
    useEffect(() => { load(); const i = setInterval(load, 5000); return () => clearInterval(i); }, []);
    const load = async () => {
        const data = await DatabaseService.getBots();
        setBots(data.filter(b => b.status === 'Active'));
    };

    const stopBot = async (id: string) => {
        if(confirm("Process'i durdurmak istediğinize emin misiniz?")) {
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">AKTİF <span className="text-blue-500">RUNTIMELAR</span></h2>
            <div className="grid grid-cols-1 gap-6">
                {bots.map(b => (
                    <div key={b.id} className="bg-[#0f172a] border border-slate-800 p-8 rounded-[40px] flex flex-col md:flex-row items-center justify-between group">
                        <div className="flex items-center gap-6">
                             ...
                             <div>
                                 ...
                                 <p className="text-[10px] text-blue-500 font-black uppercase">{b.runtime_id}</p>
                             </div>
                        </div>
                        <div className="flex-1 px-12 grid grid-cols-3 gap-8">
                            <MetricBox label="Uptime" value="100%" />
                            <MetricBox label="RAM" value={`${b.memory_usage}MB`} />
                            <MetricBox label="CPU" value={`${b.cpu_usage}%`} />
                        </div>
                ...
                {bots.length === 0 && <div className="p-20 text-center bg-slate-950/50 rounded-[40px] border border-dashed border-slate-800 text-slate-600 font-black uppercase italic">Çalışan bot bulunamadı.</div>}
            </div>
        </div>
    );
};
...
const BotManagement = () => {
    ...
    const handleAction = async (bot: BotType) => {
        if (bot.status === 'Active') {
            ...
            setIsDeploying(true);
            setTerminalLines(['[SYSTEM] Initializing Botly Engine...', '[COMPUTE] Allocating Node-A1 Virtual Machine...', '[RUNTIME] Cloning repository...']);
            
            await new Promise(r => setTimeout(r, 1000));
            setTerminalLines(p => [...p, '[BUILD] Python 3.10 environment detected.', '[BUILD] Building dependencies...', '[INFO] Polling loop established.']);
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">BOT <span className="text-blue-500">ENSTALASYONU</span></h2>
                <button onClick={() => { setEditingBot({ name: '', description: '', price: 0, category: 'productivity', bot_link: '', icon: '', python_code: 'from telegram import Update\n\n# Gerçek kod yapısı\nasync def start(update, context):\n    await update.message.reply_text("Bot Aktif!")' }); setIsModalOpen(true); }} className="px-8 py-4 bg-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white flex items-center gap-2 shadow-2xl active:scale-95 transition-all">
                    <Plus size={18} strokeWidth={3}/> Yeni Bot Üret
                </button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {bots.map(b => (
                    <div key={b.id} className="bg-[#0f172a] border border-slate-800 p-8 rounded-[48px] flex flex-col group relative overflow-hidden">
                        ...
                        <div className="flex gap-2 mt-auto relative z-10">
                            <button onClick={() => handleAction(b)} className={`flex-1 py-4 border rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${b.status === 'Active' ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-blue-600 border-blue-600 text-white'}`}>
                                {b.status === 'Active' ? <Square size={14}/> : <Play size={14}/>} {b.status === 'Active' ? 'STOP' : 'RUN'}
                            </button>
                    ...
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-[56px] p-12 relative shadow-2xl overflow-y-auto max-h-[95vh] no-scrollbar" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                            <div className="space-y-4">
                                <Input label="Bot Adı" value={editingBot.name} onChange={v => setEditingBot({...editingBot, name: v})} />
                                <Input label="Fiyat (Stars)" type="number" value={editingBot.price} onChange={v => setEditingBot({...editingBot, price: Number(v)})} />
                                <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea value={editingBot.python_code} onChange={e => setEditingBot({...editingBot, python_code: e.target.value})} className="w-full h-full min-h-[200px] bg-slate-950 border border-slate-800 rounded-2xl p-6 text-xs font-mono text-emerald-500 resize-none outline-none leading-relaxed" />
                            </div>
                        </div>
                        ...
                        {isDeploying && (
                            <div className="bg-black rounded-[32px] p-8 border border-slate-800 font-mono text-[11px] space-y-1.5 shadow-2xl mb-8 overflow-y-auto max-h-40">
                                {terminalLines.map((l, i) => <p key={i} className="text-slate-400 font-bold">{l}</p>)}
                                <div ref={terminalEndRef} />
                            </div>
                        )}

                        <button onClick={async () => { await DatabaseService.saveBotConfiguration(editingBot); setIsModalOpen(false); load(); }} className="w-full py-6 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-[32px] text-[10px] tracking-widest uppercase transition-all shadow-2xl active:scale-95">YAPILANDIRMAYI KAYDET</button>
                    </div>
                </div>
            )}
        </div>
...
const GlobalLogs = () => {
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">ROOT <span className="text-blue-500">CANLI TERMINAL</span></h2>
            <div className="bg-black border border-slate-800 rounded-[40px] p-8 shadow-2xl font-mono text-[11px] min-h-[600px] max-h-[80vh] overflow-y-auto no-scrollbar space-y-1">
                {logs.map(l => (
                    <div key={l.id} className="flex gap-4 hover:bg-slate-900/40 p-1 rounded transition-colors group">
...
const UserManagement = () => {
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">ROOT <span className="text-blue-500">KULLANICI KONTROLÜ</span></h2>
...
const AnnouncementManagement = () => {
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">MARKET <span className="text-blue-500">PROMOSYONLARI</span></h2>
